...
//	} 
		...
		if(type == 'copyinvitelink'){
			setWord(msgid, '<br>你使用的是非IE核心浏览器，请按下 Ctrl+C 复制邀请链接到剪贴板');
		} else {
			setWord(msgid, '<br>你使用的是非IE核心浏览器，请按下 Ctrl+C 复制代码到剪贴板');
		}
...
// 获取代码
function getcode(m,btn,num,pos,icon,isshow,fade){
	var isshowstr = ''
	if(typeof(isshow) != 'undefined') {
		isshowstr = '&isshow='+isshow;
	}
	var fadestr = '';
	...
	var path = (m=='index'||m=='')?'':'/'+m;
	window.location.href=SITE_URL+'/getcode'+path+'?btn='+btn+'&num='+num+'&pos='+pos+'&icon='+icon+isshowstr+fadestr;
}


function jump(m,num,showType,isshow,lkrc,colnum) {
	var isshowstr = lkrcstr ='';
	if(typeof(isshowstr) != 'undefined') {
		isshowstr = '&isshow='+isshow;
	}
	if(typeof(lkrc) != 'undefined') {
		lkrcstr = '&lkrc='+lkrc;
	}
	if(colnum != '' && colnum >0 && typeof(colnum) != 'undefined') {
		num = colnum*6;
	...
	var path = (m=='index'||m=='')?'':'/'+m;
	window.location.href=SITE_URL+'/getcode'+path+"?num="+num+"&showType="+showType+isshowstr+lkrcstr+colnum;
}



// 帮助搜索
function checkStr(input,str,restr){
	var checkString = str == "" ? restr : str;
	var inputObj = document.getElementById(input);
	if(inputObj.value == "" || inputObj.value.indexOf("请输入") > -1) inputObj.value = restr;
	if (inputObj.value  != checkString){
		document.getElementById(input).style.color = '#000000';
	}else{
		document.getElementById(input).style.color = '#CCCCCC';
	}
}
