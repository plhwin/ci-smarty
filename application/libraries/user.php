...
class User{
					...
					if($query){
						if($session = $query->row_array()) {
							...
							if($member['authpwd'] == $authpwd) {
								$setarr = array('uid'=>$uid,'nickname'=>addslashes($member['nickname']),'authpwd'=>$authpwd);
	...
	public function get_member($field, $value, $fieldname = '*'){
		if(in_array($field, array('uid', 'email'))){
	...
	public function memberlogin($setarr, $cookietime=0){
		...
		if(!is_numeric($cookietime)) {
			$cookietime = time() - 86500;
		} else {
			$cookietime = ($cookietime > 0) ? time() + $cookietime : 0;
		}
		setcookie(
			$this->CI->config->item('cookie_prefix').'auth', 
			$this->authcode("{$setarr['authpwd']}\t{$setarr['uid']}", 'ENCODE'), 
			$cookietime, 
	...
	public function insertsession($setarr) {
		...
		$session['nickname'] = $setarr['nickname'];
