...
class Check_model extends CI_Model {
     ...
     * @param $username
     */
	public function check_nickname($username){
		if(empty($username)) {
			$result = '用户昵称不能为空';
		} else{
			if(!isusername($username)){
				$result = '用户昵称中包含有特殊字符，只允许汉字，大小写字母，数字作为用户昵称';
			} else {
				$strlen = cnstrlen($username) * 2;
				if($strlen < 4 || $strlen > 20) {
					$result = '用户昵称的长度应该4-20个字符之间';
				} else {
					$result = 'OK';
				}
			}
    	...
    	if(empty($email)) {
			...
			if($member = $this->user->get_member('email', $email)){
				if($type == 1){
					if($member['emailcert']){
		    			//已经通过邮箱验证
		    			$result = '该邮箱已注册 <a href="'.SITE_URL.'/login/?memberid='.$email.'">&raquo;直接登录</a>';
		    		} else {
		    			$result = '该邮箱已注册 <a href="'.SITE_URL.'/login/?memberid='.$email.'">&raquo;直接登录</a>';
//		    			$result = '该邮箱已注册，但尚未激活。<br /><a href="'.SITE_URL.'/activate/?memberid='.$email.'">&raquo;立即激活</a>';
		    		}
				} else {
					$result = "该邮箱已经被使用，请更换";
				}
    ...
    public function check_qq($qq){
    	if(empty($qq)) {
			...
			if(is_numeric($qq)){
				$qqlen = strlen($qq);
				if($qqlen < 6 || $qqlen > 11){
					$result = 'QQ号码应该由6-16位的数字组成';
				} else {
					$result = 'OK';
				}
			} else {
				$result = 'QQ号码应该由6~11位数字组成';
			}
		}
		return $result;
    }
