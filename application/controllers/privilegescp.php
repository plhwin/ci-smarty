...
class Privilegescp extends CI_Controller {
	...
	public function editadmin($adminid){
		...
		//载入配置
		include_once APPPATH."config/adminmenu.php";
		...
		if(submitcheck('dosubmit')){
			...
			if(empty($message)){
				...
				if($result){
					...
					addAdminLog("{me} 修改了 {$setarr['adminname']} 的资料。");
	...
	public function edit($adminid){
		...
		if(submitcheck('dosubmit')){
			...
			if($result){
				...
				addAdminLog("{me} 修改了 {$adminer['adminname']} 的权限。");
		...
		if(submitcheck('dosubmit')){
			...
			if(empty($message)){
				...
				if($adminid){
					...
					addAdminLog("{me} 新增了登录名为 {$setarr['adminname']} 的管理员。");
