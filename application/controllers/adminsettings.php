...
class Adminsettings extends CI_Controller {
	...
	public function index(){
		...
		$adminuser = $this->adminusermod->get_adminuser($_SESSION['site_adminid']);		
		...
		if(submitcheck('profilesubmit')){
			...
			if(empty($message)){
				...
				updatetable('admin_user', $setarr, array('adminid'=>$_SESSION['site_adminid']));
				
				addAdminLog("{me} 修改了自己的管理员基本资料。");
	...
	public function password(){
		...
		if(submitcheck('passwordsubmit')){
			...
			//检查当前密码
			$result = $this->adminusermod->check_adminpwd_current($_SESSION['site_adminid'], $_POST['password_current']);
			if($result != 'OK'){
				$message[] = '当前'.$result;
			}
			...
			if(empty($message)){
				...
				$result = updatetable('admin_user', $setarr, array('adminid'=>$_SESSION['site_adminid']));
				if($result){
					addAdminLog("{me} 修改了自己的管理员登录密码。");
					$msgtype = 'ok';
